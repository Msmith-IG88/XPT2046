
/*******************************************************************
*
* CAUTION: This file is automatically generated by HSI.
* Version: 2022.1
* DO NOT EDIT.
*
* Copyright (C) 2010-2024 Xilinx, Inc. All Rights Reserved.
* SPDX-License-Identifier: MIT 

* 
* Description: Driver configuration
*
*******************************************************************/

#include "xparameters.h"
#include "xgpio.h"

/*
* The configuration table for devices
*/

XGpio_Config XGpio_ConfigTable[XPAR_XGPIO_NUM_INSTANCES] =
{
	{
		XPAR_AXI_GPIO_ENCODER_DEVICE_ID,
		XPAR_AXI_GPIO_ENCODER_BASEADDR,
		XPAR_AXI_GPIO_ENCODER_INTERRUPT_PRESENT,
		XPAR_AXI_GPIO_ENCODER_IS_DUAL
	},
	{
		XPAR_AXI_GPIO_BTN_DEVICE_ID,
		XPAR_AXI_GPIO_BTN_BASEADDR,
		XPAR_AXI_GPIO_BTN_INTERRUPT_PRESENT,
		XPAR_AXI_GPIO_BTN_IS_DUAL
	},
	{
		XPAR_AXI_GPIO_LED_DEVICE_ID,
		XPAR_AXI_GPIO_LED_BASEADDR,
		XPAR_AXI_GPIO_LED_INTERRUPT_PRESENT,
		XPAR_AXI_GPIO_LED_IS_DUAL
	},
	{
		XPAR_AXI_GPIO_RGBLED_DEVICE_ID,
		XPAR_AXI_GPIO_RGBLED_BASEADDR,
		XPAR_AXI_GPIO_RGBLED_INTERRUPT_PRESENT,
		XPAR_AXI_GPIO_RGBLED_IS_DUAL
	},
	{
		XPAR_SPI_DC_DEVICE_ID,
		XPAR_SPI_DC_BASEADDR,
		XPAR_SPI_DC_INTERRUPT_PRESENT,
		XPAR_SPI_DC_IS_DUAL
	},
	{
		XPAR_SPI_TOUCH_DEVICE_ID,
		XPAR_SPI_TOUCH_BASEADDR,
		XPAR_SPI_TOUCH_INTERRUPT_PRESENT,
		XPAR_SPI_TOUCH_IS_DUAL
	}
};


